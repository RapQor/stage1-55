...
const testimonial = new Promise((resolve, reject) => {
    ...
    xhr.open("GET", "https://api.npoint.io/404567c090ab26359d66", true);
    // Get -> Methode
    // yg-2 -> Alamat api
...
async function showTestimonial() {
    try {
      ...
      // Jika pake await akan tampil datanya
        ...
        testimonialHtml += `<div class="card w-25 mx-4 my-4 shadow border-0">
          <img src="${item.image}" alt="testimonial" class="card-img-top object-fit-cover border rounded" style="height: 200px;">
              <p class="card-text fst-italic mb-5">${item.content}</p>
              <p class="card-text text-end fw-bold position-absolute bottom-0 end-0">- ${item.author}</p>
              <p class="card-text text-end fw-bold position-absolute bottom-0 end-0">${item.rating} <i class="fa-solid fa-star"></i></p>
        </div>`;
        });
